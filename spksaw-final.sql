...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 12, 2025 at 07:24 PM
...
(10, 'Fadhli Rajwaa Rahmanaa', 'gbm', 2, 0, 1, 0, 2, 0, 1, '2025-09-12 08:13:32', '2025-09-12 08:13:32');
...
(3, 'Data Klasifikasi', '?module=klasifikasi', 'Data', 5, 'Y', 'admin'),
(4, 'Laporan Hasil Analisa', '?module=laporan&act=analisa', 'Report', 1, 'Y', 'admin'),
(5, 'Perankingan', '?module=perankingan', 'Report', 2, 'Y', 'admin'),
(7, 'Data Kriteria', '?module=kriteria', 'Data', 4, 'Y', 'admin'),
(8, 'Pembobotan Kriteria', '?module=pembobotan', 'Data', 3, 'Y', 'admin');
...
(10, 10, 2, 0, 1, 0, 2, 0, 1, 0, '2025-09-12 08:13:32', '2025-09-12 08:13:32');
...
(1, 'Jumlah Lansia', 0.15, 'benefit', 'Jumlah lansia dalam keluarga', '2025-09-11 06:11:16', '2025-09-12 17:12:40', 'C1', 0.06),
...
--
-- Dumping data for table `tbl_log_bobot`
--

INSERT INTO `tbl_log_bobot` (`id`, `id_kriteria`, `kode_kriteria`, `old_nilai`, `new_nilai`, `jenis`, `aksi`, `username`, `created_at`) VALUES
(1, 9, 'C9', 0.1200, 0.0000, 'benefit', 'delete', 'system', '2025-09-12 16:58:25'),
(2, 11, 'C9', 0.0000, 0.1000, 'Benefit', 'insert', 'system', '2025-09-12 16:58:37'),
(3, 11, 'C9', 0.1000, 0.0000, 'benefit', 'delete', 'system', '2025-09-12 16:58:50');
...
--
-- Stand-in structure for view `v_klasifikasi_lengkap`
-- (See below for the actual view)
--
CREATE TABLE `v_klasifikasi_lengkap` (
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `v_matriks_keputusan`
-- (See below for the actual view)
--
CREATE TABLE `v_matriks_keputusan` (
);

-- --------------------------------------------------------

--
-- Structure for view `v_klasifikasi_lengkap`
--
DROP TABLE IF EXISTS `v_klasifikasi_lengkap`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_klasifikasi_lengkap`  AS SELECT `k`.`id_klasifikasi` AS `id_klasifikasi`, `k`.`id_warga` AS `id_warga`, `w`.`nama_lengkap` AS `nama_lengkap`, `w`.`alamat` AS `alamat`, `k`.`id_kriteria` AS `id_kriteria`, `kr`.`kriteria` AS `kriteria`, `kr`.`bobot` AS `bobot`, `k`.`nilai_input` AS `nilai_input`, `k`.`nilai_konversi` AS `nilai_konversi`, `nk`.`keterangan` AS `nilai_keterangan` FROM (((`tbl_klasifikasi` `k` join `data_warga` `w` on(`k`.`id_warga` = `w`.`id_warga`)) join `tbl_kriteria` `kr` on(`k`.`id_kriteria` = `kr`.`id_kriteria`)) join `tbl_nilai_kriteria` `nk` on(`k`.`id_nilai` = `nk`.`id_nilai`)) ;

-- --------------------------------------------------------

--
-- Structure for view `v_matriks_keputusan`
--
DROP TABLE IF EXISTS `v_matriks_keputusan`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_matriks_keputusan`  AS SELECT `w`.`id_warga` AS `id_warga`, `w`.`nama_lengkap` AS `nama_lengkap`, max(case when `k`.`id_kriteria` = 1 then `k`.`nilai_konversi` end) AS `lansia`, max(case when `k`.`id_kriteria` = 2 then `k`.`nilai_konversi` end) AS `disabilitas`, max(case when `k`.`id_kriteria` = 3 then `k`.`nilai_konversi` end) AS `anak_sd`, max(case when `k`.`id_kriteria` = 4 then `k`.`nilai_konversi` end) AS `anak_smp`, max(case when `k`.`id_kriteria` = 5 then `k`.`nilai_konversi` end) AS `anak_sma`, max(case when `k`.`id_kriteria` = 6 then `k`.`nilai_konversi` end) AS `balita`, max(case when `k`.`id_kriteria` = 7 then `k`.`nilai_konversi` end) AS `ibu_hamil` FROM (`data_warga` `w` left join `tbl_klasifikasi` `k` on(`w`.`id_warga` = `k`.`id_warga`)) GROUP BY `w`.`id_warga`, `w`.`nama_lengkap` ;
...
--
-- Indexes for table `tbl_log_bobot`
--
...
--
-- AUTO_INCREMENT for table `tbl_log_bobot`
--
ALTER TABLE `tbl_log_bobot`
