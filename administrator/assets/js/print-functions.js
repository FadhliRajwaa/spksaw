...
function printWarga() {
    ...
    rows.forEach(function(row, index) {
        ...
        if (cells.length >= 10) { // Ensure we have enough columns
            ...
            var lansiaText = lansia > 0 ? `${lansia} orang` : '-';
            var disabilitasText = disabilitas > 0 ? `${disabilitas} orang` : '-';
            var anakSDText = anakSD > 0 ? `${anakSD} anak` : '-';
            var anakSMPText = anakSMP > 0 ? `${anakSMP} anak` : '-';
            var anakSMAText = anakSMA > 0 ? `${anakSMA} anak` : '-';
            var balitaText = balita > 0 ? `${balita} anak` : '-';
            var ibuHamilText = ibuHamil > 0 ? `${ibuHamil} orang` : '-';
            ...
            tableRows += `
                <tr>
                    ...
                    <td class="criteria-cell">${lansiaText}</td>
                    <td class="criteria-cell">${disabilitasText}</td>
                    <td class="criteria-cell">${anakSDText}</td>
                    <td class="criteria-cell">${anakSMPText}</td>
                    <td class="criteria-cell">${anakSMAText}</td>
                    <td class="criteria-cell">${balitaText}</td>
                    <td class="criteria-cell">${ibuHamilText}</td>
                </tr>
            `;
        }
    });
    ...
    <head>
        ...
        <style>
            * {
            ...
            .summary-card.total { border-color: #3b82f6; background: linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%); }
            .summary-card.lansia { border-color: #f59e0b; background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); }
            .summary-card.disabilitas { border-color: #ef4444; background: linear-gradient(135deg, #fee2e2 0%, #fecaca 100%); }
            .summary-card.anak { border-color: #10b981; background: linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%); }
        ...
        <div class="document-container">
            <div class="print-header">
                <div class="header-content">
                    ...
                    <p><strong>Dinas Sosial Republik Indonesia</strong></p>
                    <p>Sistem Pendukung Keputusan - Metode Simple Additive Weighting</p>
                    <div class="header-badge">Dicetak pada: ${new Date().toLocaleString('id-ID')}</div>
                </div>
            </div>
            
            <div class="executive-summary">
                <div class="summary-title">📊 Ringkasan Data Warga PKH</div>
                <div class="summary-cards">
                    <div class="summary-card total">
                        <h3>${totalWarga}</h3>
                        <p>Total Warga</p>
                    </div>
                    <div class="summary-card lansia">
                        <h3>${totalLansia}</h3>
                        <p>Total Lansia</p>
                    </div>
                    <div class="summary-card disabilitas">
                        <h3>${totalDisabilitas}</h3>
                        <p>Penyandang Disabilitas</p>
                    </div>
                    <div class="summary-card anak">
                        <h3>${totalAnakSD + totalAnakSMP + totalAnakSMA + totalBalita}</h3>
                        <p>Total Anak & Balita</p>
                    </div>
                </div>
            ...
            <div class="table-container">
                <div class="table-title">📋 Detail Data Warga dan Kriteria PKH</div>
                <table>
                    <thead>
                        <tr>
                            ...
                            <th colspan="7" style="background: linear-gradient(135deg, #dc2626 0%, #b91c1c 100%);">Kriteria Penilaian PKH</th>
                        </tr>
                        <tr>
                            <th>Lansia<br><small>(≥60 tahun)</small></th>
                            <th>Disabilitas<br><small>(Berkebutuhan Khusus)</small></th>
                            <th>Anak SD<br><small>(6-12 tahun)</small></th>
                            <th>Anak SMP<br><small>(13-15 tahun)</small></th>
                            <th>Anak SMA<br><small>(16-18 tahun)</small></th>
                            <th>Balita<br><small>(0-5 tahun)</small></th>
                            <th>Ibu Hamil<br><small>(Mengandung)</small></th>
                        </tr>
                    </thead>
                    <tbody>
                        ${tableRows}
            ...
            <div class="print-footer">
                <div class="footer-content">
                    ...
                    <div class="footer-center">
                        <strong>© ${new Date().getFullYear()} Dinas Sosial RI</strong><br>
                        Sistem PKH Digital v2.0
                    </div>
                    <div class="footer-right">
                        <strong>Total Data:</strong><br>
                        ${totalWarga} Warga Terdaftar
                    </div>
                </div>
                
                <div class="footer-disclaimer">
                    <strong>KETERANGAN:</strong> Dokumen ini berisi data warga calon penerima Program Keluarga Harapan (PKH) 
                    yang telah melalui proses verifikasi dan validasi sesuai dengan kriteria yang ditetapkan. 
                    Data ini bersifat rahasia dan hanya digunakan untuk kepentingan administrasi program PKH. 
                    <strong>Dilarang menyebarluaskan tanpa izin resmi dari Dinas Sosial.</strong>
                </div>
            </div>
        </div>
    </body>
